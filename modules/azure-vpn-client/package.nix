...
let
  ...
  # Drop the ambient caps inherited from the security wrapper.
  # This is required so the desktop portal can introspect the process and open the file picker.
  zenityDroppedAmbient = writeShellScriptBin "zenity" ''
    exec ${libcap.out}/bin/capsh --noamb -- -c '${zenity}/bin/zenity "$@"' zenity-shim "$@"
  '';
